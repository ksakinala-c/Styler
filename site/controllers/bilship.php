...
class bilship extends Controller 
{
	...
	{		
			...
			'wallet_amount' => $this->session->userdata('mywalletdata'),
			 ...
			 //$this->bilship_model->updatemeasure($arrOrderId);  
						...
						foreach( $this->cart->contents() as $arrRowDeailts )  
						{
							...
							if($arrRowDeailts['options']['currency'] !=""){
								$currencycode = $this->home_model->getcurrencyname($arrRowDeailts['options']['currency']);
								//$currencycode =  $arrRowDeailts['options']['currency'];
							} else {
							...
							$arrData=array(
									...
									'order_item_currency'=> $currencycode,
									'order_status'=> $order_status,
									'sizeid'=> $sizeid,
		...
		if($id == '3'){
			redirect($this->config->item('base_url').'home/zeroordersuccess');	
		}
