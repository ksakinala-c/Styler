<?php
	...
	function addcart3d(){
		...
		$cartprod = array(
			   ...
			   'price'   => $price, 
			   'name'    => $pname,
			   'options' => array('details'=>$details , 'imagename'=>$timeimage.".png", 'is_3d'=>'1') 
 	    );
		...
		$saveid = $this->cart_model->addto3dinsert($data);
	...
	function measure()
	{
		...
		$data['proparts'] = $this->home_model->prodparts($data['subcat']);
	...
	function fit($style_id)
	{
		...
		$subcatid = $this->session->userdata('subcat');
		$data['proparts'] = $this->home_model->prodparts($subcatid);
		$data['subcat']=$subcatid ;
		//print_r($data['proparts']);die;
		$data['title'] = 'Stylior.com';
