...
/*	function index()
	...
	function index()
	{
		...
		$this->load->view('newindex.php',$data);
	}
	
	function customHtml()
	{
		$sql = "SELECT * from tbl_product where is_trail_shirt = 1";
		/*$sql = "select ps.* , p.*  from tbl_product p 
		left join   tbl_product_image ps ON p.id = ps.pid where p.id <> 0  and p.qty > 0 and p.is_trail_shirt =1";*/
		...
		/*echo "<pre>";
		print_r($data);exit;*/
		$this->load->view('custom.php',$data);
	}
...
else // user logged in 
{
	...
	function contact()
	{
		...
		if($this->input->post('action') == 'subscribe') 
		{
				...
				$headers .= 'Cc: info@stylior.com' . "\r\n";
 				mail('support@stylior.com,'.$this->input->post('email'), $subject, $message, $headers); 
	...
	function lists() 
 	{
		...
		$data['keywords'] = '';
		$data['description'] = '';
		$this->load->view('category2.php',$data);
	}
	 ...
	 function mensshirts() 
 	{
		...
		$data['size'] = $this->input->get('size');
		$data['price'] = $this->input->get('price');
		$data['s_price'] = $this->input->get('s_price');
		$data['e_price'] = $this->input->get('e_price');
		$data['sortby'] = $this->input->get('sortby');
		$data['page'] = $this->input->get('page');
		$data['discount1'] = $this->input->get('discount1');
		$data['designid'] = $this->input->get('designid');
		...
		$config['base_url'] = $url_to_paging.'home/lists/?per_page='.$page.'&title='.$this->input->get('search').'&sortby='.$this->input->get('sortby').'&page='.$data['page'].'&discount1=&cat='.$this->input->get('cat').'&subcat='.$this->input->get('search').'&search='.$this->input->get('search').'&discount='.$this->input->get('discount').'&color='.$this->input->get('color').'&fabric='.$this->input->get('fabricid').'&design='.$this->input->get('designid').'&'.$sizerange;
		$return = $this->home_model->allproductsNew($config['per_page'],$pageno, $data);
		$data['allproducts'] = $return['result'];
		$config['total_rows'] = $return['count'];
		$data['images'] = $return['images'];
		$this->pagination->initialize($config);
		...
		if ($this->validation->run() == FALSE){
			$data['L_strErrorMessage'] = $this->validation->error_string;
				...
				$message = '<div style="width:700px; height:auto; margin:0 auto;">
					...
					<table>
							<tr><td>Email:</td><td>'.$email.'</td></tr>
							<tr><td>Reset Password Link:</td><td><a href='.$base_url.'home/resetpassword/'.$password.'>Click Here</a></td></tr>
					...
					//echo $message;die;
					$data['msg'] = 'Reset password link has been successfully sent to your email id. please check Your Mail.!!';
	...
	function details_temp($id,$prodid)
	{
		$data = array(); 
		$data['L_strErrorMessage'] = "";
		$data['err_msg'] = ""; 
...
function step($tab_id=0,$shipid = '0')
	...
	{ 
		...
		//print_r($data['inc_default_val']);die;
	...
	if($this->input->post('action') == 'resetpwd') {
	foreach($form_field as $key => $value)	{	$data[$key]=$this->input->post($key);	}
		
		
		$rules['upwd']       = "trim|required";
		
		$this->validation->set_rules($rules);
		$fields['upwd']       = "Password";
		$this->validation->set_fields($fields);
		if ($this->validation->run() == FALSE){
			$data['L_strErrorMessage'] = $this->validation->error_string;
		} else {
				if($response['data'] = $this->home_model->user_mail_exist1($userid))
				{
					
						$this->home_model->passwordupdate($this->input->post('upwd'),$userid); 
						$data['msg'] = 'Your password has been reset successfully.!!';
						$data['title'] = 'Reset-Password Stylior.com';
						$data['keywords'] = '';
						$data['description'] = '';
						//$data['msg'] = '';
					$this->load->view("resetpwd",$data);
						...
						$data['description'] = '';
						$data['msg'] = '';
						$this->session->set_flashdata('L_strErrorMessage','Your Email Address is Wrong Please try again.!!');
						redirect($this->config->item('base_url').'home/resetpassword/'.$userid.'', 'location');
						//$this->load->view("resetpwd",$data);
				}
			}
		} else {
	...
	/*
	 * action which will allow for guest user can save their measurement
	 * @Author MYSYS009
	*/
	function guestCheckOut()
	{
		$this->load->view('user_basic_info');
	}
}
?>