...
class Home_Model extends Model 
{
	...
	function allproducts($pg_num, $offset, $content) 
	{
		...
		/*$sql = "SELECT p.* , im.image from  tbl_product p
			...
			where p.id <> 0";
			//Commented by Ms001
			select ps.* , p.* , im.image ,ps.id as proid from pro_style ps 
		inner join tbl_product p ON p.id = ps.pid
		left join tbl_style_image im ON im.style_id = ps.id AND im.baseimage = '1'  where p.id <> 0
			$sql = "select ps.* ,pm.image as fimage  from tbl_product ps
		left join tbl_product_image pm  on ps.id=pm.pid";
			*/

		
		$sql = "select ps.* , p.*  ,ps.id as proid from tbl_product p 
		left join  pro_style ps ON p.id = ps.pid where p.id <> 0";
		...
		if($content['price'] != ''){
		    ...
		    if($content['price'] == '2000'){
					$sql .= " AND p.price <= 2000 "; 
			}
			if($content['price'] == '2500'){
					$sql .= " AND ( p.price > 2000 AND p.price <= 2500 ) "; 
			}
			if($content['price'] == '3000'){
					$sql .= " AND ( p.price > 2500 AND p.price <= 3000 ) "; 
			}
			if($content['price'] == '3500'){
					$sql .= " AND ( p.price > 3000 AND p.price <= 3500 ) "; 
			}
			if($content['price'] == '4000'){
					$sql .= " AND ( p.price > 3500 AND p.price <= 4000 ) "; 
			}
		...
		$sql_couint = "select ps.* , im.image ,ps.id as proid from tbl_product ps
		left join tbl_product_image im ON im.pid = ps.id";
		
		if($content['cat'] != ''){
			$sql_couint .= " AND  ps.categoryid = '".$content['cat']."' ";  
		}
		
		if($content['subcat'] != ''){
			$sql_couint .= " AND find_in_set('".$content['subcat']."',ps.subcatid) <> 0"; 
		}
		
		if($content['cup'] != ''){
			$sql_couint .= " AND ps.cup = '".$content['cup']."'";
		}
		
		
		if($content['size'] != '' && count($content['size']) > 0){
			$sql_couint .=	"  AND  (ps.size IN (".implode(',',$content['size']).") )  ";  
		}
		
		if($content['fabricid'] != ''){
			$sql_couint .= " AND  ps.fabricid = '".$content['fabricid']."' "; 
		}
		
		if($content['designid'] != ''){
			$sql_couint .= " AND  ps.designid = '".$content['designid']."' "; 
		}
		
		
		if($content['color'] != ''){
			$sql_couint .= " AND ps.colour = '".$content['color']."'";
		}
		
		if($content['search'] != ''){
			//$sql_couint .= " AND p.pname like '%".$content['search']."%'"; 
			$sql_couint .= " AND ( ps.pname like '%".$content['search']."%' OR  ps.foldername like '%".$content['search']."%'  ) "; 
		}
        
		if($content['discount'] != ''){
			$sql_couint .= " AND ps.discount > '".$content['discount']."'"; 
		}
		
		if($content['price'] != ''){
		    if($content['price'] == '2000'){
					$sql_couint .= " AND ps.price <= 2000 "; 
			}
			if($content['price'] == '2500'){
					$sql_couint .= " AND ( ps.price > 2000 AND ps.price <= 2500 ) "; 
			}
			if($content['price'] == '3000'){
					$sql_couint .= " AND ( ps.price > 2500 AND ps.price <= 3000 ) "; 
			}
			if($content['price'] == '3500'){
					$sql_couint .= " AND ( ps.price > 3000 AND ps.price <= 3500 ) "; 
			}
			if($content['price'] == '4000'){
					$sql_couint .= " AND ( ps.price > 3500 AND ps.price <= 4000 ) "; 
			}
			
		}
		$sql_couint .= " group by ps.id ";
 		$query1 = $this->db->query($sql_couint);
		$ret['result'] = $query->result();
		$product_images_pids=array();
	...
	function allproductsNew($pg_num, $offset, $content) 
	{
		if($offset == ''){
			$offset = '0';
		}
		...
		// added this line in the query   "and p.qty > 0" by MSYS009
		$sql = "select ps.* , p.*  ,ps.id as proid from tbl_product p 
		left join  pro_style ps ON p.id = ps.pid where p.id <> 0  and p.qty > 0 and p.is_trail_shirt !=1";
		...
		if($content['color'] != ''){
			$sql .= " AND (p.colour IN (".implode(',',$content['color']).") )  ";
		}
		...
		if($content['fabricid'] != ''){
			$sql .= " AND (p.fabricid IN (".implode(',',$content['fabricid']).") )  ";
		}
		
		if($content['designid'] != ''){
			$sql .= " AND  (p.designid IN (".implode(',',$content['designid']).") )  "; 
		}
		...
		if($content['s_price'] != '' && $content['e_price'] != ''){
		   ...
		   /* if($content['price'] == '2000'){
			...
			if($content['price'] == '4000'){
					...
					$sql .= " AND ( p.price > '".$content['s_price']."' AND p.price <=  '".$content['e_price']."' ) "; 
	...
	function productdetailsold($id)
	{ 
	    echo $sql = "SELECT p.*, fb.fabricsname as name, im.image from  pro_style p 
		        left join  fabrics fb ON fb.id = p.style_id
 			    left join  tbl_style_image im ON im.style_id = p.id
 				AND im.baseimage = '1' where p.id = ".$id;die;
 		$query = $this->db->query($sql);
		if ($query->num_rows() > 0)
		{
			$result = $query->row();
	...
	function productinfo($id){
		$sql = "select a.*,b.colourname,c.fname,d.designname from tbl_product a
			inner join colour b on ( b.id = a.colour )
			inner join newfabric c on ( c.id = a.fabricid )
			inner join design d on ( d.id = a.designid ) where a.id ='".$id."'";
		$query = $this->db->query($sql);
