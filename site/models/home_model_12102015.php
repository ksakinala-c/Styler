...
class Home_Model extends Model 
{
	...
	{
		//echo $id;
		$sql = "SELECT p.*  FROM  add_parts p where p.body_measure = '1' ";  
	...
	{ 
	    /*$sql = "SELECT f.*, p.*, im.image from fabrics f 
			  left join tbl_product p ON p.id = f.pid
			  left join tbl_style_image im ON im.pid = p.id AND im.baseimage = '1'
 			  where f.id = ".$id;*/
		$sql = "select ps.* , p.*, im.image ,ps.id as proid from pro_style ps 
		left join tbl_product p ON p.id = ps.pid
		 left join tbl_style_image im ON im.pid = p.id AND im.baseimage = '1'
		where ps.style_id = '".$id."'";
...
function getadd($user_id)
  ...
  $sql = "SELECT * FROM shipping_address where user_id = '".$this->session->userdata('userid')."'  AND Status = '1' ";
