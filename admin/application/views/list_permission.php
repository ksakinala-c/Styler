...
<script type="text/javascript">
	$(document).ready(function($) {
		...
		$('#reset').click(function(){
				var base_url = '<?php echo $base_url.'permission/list_permission'; ?>';
		...
		$(function() {
		$.ajax({
			url:'<?php echo $base_url; ?>permission/ajaxlist_permission',
			type:'POST',
			data:'',
...
<div class="main-content theme">
  <ol class="breadcrumb bc-3">
    ...
    <li class="active"><strong>List User Permission </strong></li>
  </ol>
<?php if(isset($_POST))
{
if(count($result)==0)
...
{
?>
<a href="<?php echo $base_url;?>permission/list_permission"></a>
<?php
}
?>
<a class="btn btn-third add" href="javascript:void('0');" onclick="deletelocation();"><i class="entypo-plus"></i>Delete</a>
<?php if(isset($_POST))
{
	if(count($result)==0)
	{
	?>
	<a class="btn btn-primary add" href="<?php echo $base_url;?>permission/add"><i class="entypo-plus"></i>Add Permission</a>
	<?php
	}
	else
	{
...
<div class="row">
<div class="col-xs-12 col-sm-12"><h6 class="main-title">List Permission</h6></div>
     <div class="col-6 col-sm-12 col-lg-12 action">
      <div class="end">
        <form action="<?php echo $base_url."permission/list_permission";?>" method="post" enctype="multipart/form-data" id="form1"  class="pro"  autocomplete="on">
          <div class="form-group mock">
            <label class="col-sm-1 control-label lavel" for="inputEmail">User Type :</label>
            <div class="col-lg-3">
           <input type="text" id="cname" name="cname"  value="<?php echo $cname; ?>" class="form-control jobtext from-dainn"   />
            </div>
			<div class="col-lg-5">
			<div class="set1">
      ...
      <table id="table-2" class="table table-bordered table-striped datatable dataTable" aria-describedby="table-2_info" style="float:left;">
        <thead>
          <tr role="row" class="bg">
			...
			<th class="sorting" role="columnheader" aria-controls="table-2" style="width: auto;">Permission</th>
			<!--th class="sorting" role="columnheader" aria-controls="table-2" style="width: auto;">Edit Permission</th-->
        ...
        <tbody role="alert" aria-live="polite" aria-relevant="all">
          ...
          <tr class="odd">
           ...
           <td class=""><?php echo $result[$i]->cname; ?></td>
		    <td class=""> <?php echo $this->load->permission_model->permision($result[$i]->permission); ?></td>
			<?php /* <td class=""> <?php if($this->load->permission_model->editpermision($result[$i]->editperm) !=""){ echo $this->load->permission_model->editpermision($result[$i]->editperm); } else { echo "NOT UPDATED YET"; } ?></td> */ ?>
			...
			<td class=" ">
				<a class="btn btn-default btn-sm btn-icon icon-left" title="Edit" href="<?php echo $base_url."permission/edit/"; ?><?php echo $result[$i]->id; ?>">
          ...
          <?php
                  ...
                  ?>
          <tr>
            <td class="left" colspan="4" align="center"><?php echo 'No Permission Found.!!'; ?></td>
          </tr>
          <?php
                  }
                  ?>
