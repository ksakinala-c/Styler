		...
		if(this.vpb_files.length > 0)
		{
 			...
 			for(var i = 0; i<this.vpb_files.length; i++)
			{
				...
				if(typeof this.vpb_files[i] != undefined && this.vpb_files[i].name != "")
				{
					...
					vpb_added_files_displayer += '<div id="add_fileID'+vpb_file_id+'" class="'+new_classc+' row"><div id="vpb_files_left" class="hove_this_link span8"><div class="row"><div class="span6">'+vpb_file_icon+' '+this.vpb_files[i].name.substring(0, 40)+'</div><div class="span2"><span id="uploading_'+vpb_file_id+'"><span class="ready">Ready</span></span></div></div></div><div id="vpb_files_size_left" class="span1">'+vpb_actual_fileSize+'</div><div id="vpb_files_time_left" class="span2">'+vpb_date_last_modified_file+'</div><div id="vpb_files_remove_left" class="span1"><span id="remove'+vpb_file_id+'"><span class="vpb_files_remove_left_inner" onclick="vpb_remove_this_file(\''+vpb_file_id+'\',\''+this.vpb_files[i].name+'\');">Remove</span></span></div></div>';
	...
	vpb_multiple_file_uploader.prototype.vasPLUS = function(file,file_counter)
	{
		if(typeof file[file_counter] != undefined && file[file_counter] != '')
		{
			...
			else
			{
                ...
                dataString.append('wsz_csrf',$('input[name=wsz_csrf]').val());
                dataString.append('submit',true);
