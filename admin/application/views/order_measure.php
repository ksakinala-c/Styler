...
<div class="main-content theme">
  <ol class="breadcrumb bc-3">
    <li><a href="<?php echo $base_url;?>home"><i class="entypo-arrow"></i>Order </a></li>
...
<div class="col-xs-12 col-sm-12"><h6 class="main-title">Measure Data Details
  <button class="btn btn-default" style="float:right;" onclick="javascript:window.location.href='<?php echo $base_url;?>product/order_detail/<?php echo $this->uri->segment(3); ?>'">Cancel</button>
</h6>
	  ...
	  <table id="table-2" class="table table-bordered table-striped datatable dataTable" aria-describedby="table-2_info" style="float:left;">
        <thead>
          <tr role="row" class="bg">
			...
			<td><b>Weight</b></td>
			<td><b>Posture</b></td>
			<td><b>Shoulder Type</b></td>
			<td><b>Shoulder Angle</b></td>
			<td><b>Fit</b></td>
			<td><b>Brand</b></td>
			<td><b>Measurements</b></td>
		    <td><b>3d Measurement</b></td>
        ...
        <tbody role="alert" aria-live="polite" aria-relevant="all">
		 <?php  
		 $ci_item_data = $this->product_model->getmdetail($this->uri->segment(3));
		// echo "<pre>";print_r( $ci_item_data);die;
		
		if($ci_item_data != '' && count($ci_item_data) > 0) {  
		foreach($ci_item_data as $mdata)
		{
		$mesuredetail = $this->product_model->getmeasuredetail($mdata->measureid);
		if($mesuredetail != '' && count($mesuredetail) > 0) {  
		foreach($mesuredetail as $measure)
		{
			// echo "<pre>";print_r($measure);
		 ?>
		<tr class="odd">
		 
 			<?php if($mdata->save3d == '1') { ?>
					<td class=" "> 
Product Name: <?php echo $mdata->order_item_name;?>
				</td>
			<?php } else { ?>
				<td class=" "> Product Name:<?php echo $this->users_model->getproname($measure->pid); ?> 
				</td>
			<?php } ?>


			  <td><?php echo $measure->metricft; ?> feet <?php echo $measure->metricinch; ?> Inches  </td>
			 <td><?php echo $measure->metricweight; ?> </td>
			 <td class=""><?php 
			 ...
			 <td> <?php 
								...
								if($post == "3") { echo "Slim"; }	
								?> </td>
								<td class=""><?php echo $this->users_model->getbname($measure->brandid); ?></td>
			 <td> <?php 
				  ...
				  ?> </td>
				<td> <?php if($mdata->save3d == '1') { ?>
				  
				  <p>3D Parameter:<table  border="0">
										<?php
  					$arr3d = json_decode($mdata->details3d); 
 					?>
										    <tr>
