...
#fff;    float: right;    font-size: 13px;       line-height: 20px;    margin: 0px 0 0;    padding: 7px;}" onclick="myFunction()">Print Invoice</button>   
        ...
        <div style="clear:both;"></div>
         ...
         <div class="col-xs-6">
          <div class="panel panel-default">
            ...
            <div class="panel-body">
              <p style="font-size:16px;">
              <?php echo $shippingdetails->sname.",<br/>".$shippingdetails->address.",<br/>".$shippingdetails->address1.",<br/>".$shippingdetails->city.",".$shippingdetails->county.",<br/>".$shippingdetails->postcode; ?>
              </p>
            </div>
          </div>
        </div>
        <div class="col-xs-6 text-right">
          <div class="panel panel-default">
            ...
            <div class="panel-body" style="min-height:154px;">
              <p style="font-size:16px;">
               <?php echo  $shippingdetails->sname.",<br/>".$shippingdetails->saddress.",<br/>".$shippingdetails->saddress1.",<br/>".$shippingdetails->scity.",<br/>".$shippingdetails->scounty.",<br/>".$shippingdetails->spostcode; ?>
            </div>
          </div>
        </div>
      </div>
      ...
      <table class="table table-bordered">
        <thead>
          <tr>
            ...
            <th>
              <h4>Price (<?php echo $orderdetails->order_item_currency; ?>)</h4>
            </th>
            <th>
              <h4>Unit</h4>
            </th>
            ...
            <th>
              <h4>Total (<?php echo $orderdetails->order_item_currency; ?>)</h4>
            </th>
            <th>
              <h4>Tax</h4>
            </th>
     ...
     <?php 
	 $c = $this->product_model->currencydetails($orderdetails->order_item_currency);
	...
	/* if($orderitems != '' && count($orderitems) > 0)
          ...
          <tr>
				...
				Product Name: <?php echo $orderdetails->order_item_name;?></td>
				<td>
				
				<?php
				if($orderdetails->order_item_currency == 'INR')
				{ ?>
						INR <?php echo number_format($orderdetails->product_item_price,'2','.','');?> 
				<?php } else { 
 						echo $orderdetails->order_item_currency." "; 
						echo roundUpToAny(( $orderdetails->product_item_price / ( $cvalue * ($multiplier/100)) )); 
				} ?>
				</td>
				<td><?php echo $orderdetails->order_item_currency; ?></td>
				<td><?php echo $orderdetails->product_quantity; ?></td>
				<td> 
 				<?php
				if($orderdetails->order_item_currency == 'INR')
				{ ?>
						INR <?php echo number_format( ( $orderdetails->product_item_price * $orderdetails->product_quantity ),'2','.','');?>
				<?php } else { 
 						echo $orderdetails->order_item_currency." "; 
						echo roundUpToAny(( $orderdetails->product_item_price * $orderdetails->product_quantity / ( $cvalue * ($multiplier/100)) )); 
				} ?>
