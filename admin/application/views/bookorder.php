...
#fff;    float: right;    font-size: 13px;       line-height: 20px;    margin: 0px 0 0;    padding: 7px;}" onclick="myFunction()">Print Book Order</button>   
         ...
         <div class="col-xs-6">
          <div class="panel panel-default">
            ...
            <div class="panel-body">
              ...
              <br />
			   Mobile: <?php echo $shippingdetails->mobno; ?>
			  </p>
            </div>
          </div>
        </div>
	 ...
	 if($orderitems != '' && count($orderitems) > 0)
	 {
          ...
          <tr>
			...
			<?php } ?>
				...
				{ ?>
						INR <?php echo number_format($orderitems1->product_item_price,'2','.','');?> 
				<?php } else { 
 						echo $orderdetails->order_currency." "; 
						echo roundUpToAny(( $orderitems1->product_item_price / ( $cvalue * ($multiplier/100)) )); 
				} ?></td>
				...
				if($orderdetails->order_currency == 'INR')
				{ ?>
						INR <?php echo number_format( ( $orderitems1->product_item_price * $orderitems1->product_quantity ),'2','.','');?>
				<?php } else { 
 						echo $orderdetails->order_currency." "; 
						echo roundUpToAny(( $orderitems1->product_item_price * $orderitems1->product_quantity / ( $cvalue * ($multiplier/100)) )); 
				} ?></td>
     ...
     <?php if($orderitems != '' && count($orderitems) > 0)
		  ...
		  <?php if($ord1->save3d == '1') { ?>
			...
			<?php } ?>
				...
				<td><?php 
						...
						if($measurement->type == '4') { echo "Brand Sizes"; } ?>
						<br/> <?php
						...
						Brand:  <?php echo $this->product_model->brandsize($measurement->brandid); ?> <br/> 	
						Fit: <?php echo $this->product_model->fitmeasure($measurement->fitid); ?><br/> 	
						Size: <?php echo $this->product_model->sizemeasure($measurement->sizeid); ?><br/> 	
		  ...
		  <?php }} ?>
			...
			<td colspan="5" >(<?php echo $orderdetails->order_currency ; ?>)<?php echo $orderdetails->order_total ; ?></td>
		  </tr>
        </tbody>
      </table>
