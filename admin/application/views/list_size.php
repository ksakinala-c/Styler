...
<script type="text/javascript">
	...
	$(document).ready(function($)
	{
		...
		$('#reset').click(function(){
			...
			var base_url = '<?php echo $base_url.'size/lists/'.$this->uri->segment(3) ?>';
		...
		$.ajax({
			url:'<?php echo $base_url; ?>size/ajaxlist_size',
			type:'POST',
			data:'',
...
{
?>
<a href="<?php echo $base_url;?>size/lists"></a>
<?php
}
?>
...
{
?>
<a class="btn btn-primary add" href="<?php echo $base_url;?>size/add/<?php echo $this->uri->segment(3); ?>"><i class="entypo-plus"></i>Add size</a>
<?php
}
else
{
...
<div role="grid" class="dataTables_wrapper form-inline" id="table-2_wrapper">
 ...
 <div class="col-xs-12 col-sm-12"><h6 class="main-title">List size</h6></div>
    ...
    <div class="col-6 col-sm-12 col-lg-12 action">
      <div class="end">
        <form action="<?php echo $base_url."size/lists/".$this->uri->segment(3); ?>" method="post" enctype="multipart/form-data" id="form1"  class="pro">

		<div class="form-group mock">

            <label class="col-lg-1 control-label lavel" for="inputEmail">size Name:</label>
            <div class="col-lg-3">
            <input type="text" id="size" name="size" value="<?php echo $size;?>" class="form-control jobtext"/>
            </div>
...
<?php }  ?>
  ...
  <div role="grid" class="dataTables_wrapper form-inline" id="table-2_wrapper">
    <form action="<?php echo $base_url."size/deletes/".$this->uri->segment(3)." ";?>" method="post" enctype="multipart/form-data" id="form">
      ...
      <table id="table-2" class="table table-bordered table-striped datatable dataTable" aria-describedby="table-2_info" style="float:left;">
        <thead>
          <tr role="row" class="bg">
			...
			<th class="sorting" role="columnheader" aria-controls="table-2" style="width: auto;">size Name</th>
          ...
          <tr class="odd">
            <td class="  sorting_1"><div class="checkbox checkbox-replace neon-cb-replacement">
                <label class="cb-wrapper">
			 	...
			 	<td class=" "><?php echo $this->size_model->getcatname($result[$i]->subcatid); ?></td>
		 
			<td class=" "><?php echo $result[$i]->size; ?></td> 
			 <td class=" "><?php $mpart = $result[$i]->fit; 
			 if($mpart == '1') { echo "Regular"; }
			 if($mpart == '2') { echo "Tailored"; }
			 if($mpart == '3') { echo "Slim"; }
			 ?></td> 
			...
			<td class=" ">
			  <a class="btn btn-default btn-sm btn-icon icon-left" title="Add Image" href="<?php echo $base_url."size/editimage/".$result[$i]->id.'/'.$this->uri->segment(3); ?>" >
                     <i>Add Image</i>
                     </a> &nbsp;
				<a class="btn btn-default btn-sm btn-icon icon-left" title="Edit" href="<?php echo $base_url."size/edit/"; ?><?php echo $result[$i]->id; ?>/<?php echo $this->uri->segment(3); ?>">
