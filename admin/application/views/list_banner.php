...
<a class="btn btn-primary add" href="<?php echo $base_url;?>banner/add"><i class="entypo-plus"></i>Add Banner</a>
...
<form action="<?php echo $base_url."banner/deletes/";?>" method="post" enctype="multipart/form-data" id="form">
...
<table id="table-2" class="table table-bordered table-striped datatable dataTable" aria-describedby="table-2_info">
	<thead>
		<tr role="row" class="bg">
			...
			<th class="sorting" role="columnheader" aria-controls="table-2" style="width: auto;">Caption</th>
			<th class="sorting" role="columnheader" aria-controls="table-2" style="width: auto;">Position</th>
	...
	<tbody role="alert" aria-live="polite" aria-relevant="all">
		...
		<tr class="odd">
			<td class="  sorting_1">
			 ...
			 <td class=""><img src="<?php echo $front_base_url;?>upload/banner/small/<?php echo $result[$i]->image;?>" /></td>
			<td class=" "><?php if($result[$i]->order !="" && $result[$i]->order >0){ echo $result[$i]->order; } else { echo "Not Updated"; } ?></td>
			<td class=" "><?php echo $result[$i]->caption; ?></td>
 			<td class=" "><?php if($result[$i]->homepage == '1') { ?> Home Page <?php } ?>
			<?php if($result[$i]->first_row == '1') { ?> First Row <?php } ?>
			<?php if($result[$i]->second_row == '1') { ?> Second Row <?php } ?>
			<?php if($result[$i]->third_row == '1') { ?> Third Row <?php } ?>
			</td>
 			<?php
			...
			<td class=" ">
				<a class="btn btn-default btn-sm btn-icon icon-left" title="Edit" href="<?php echo $base_url."banner/edit/"; ?><?php echo $result[$i]->id; ?>">
		...
		<?php
            ...
            ?>
            <tr>
                <td class="left" colspan="8" align="center"><?php echo 'No Banner Found'; ?></td>
            </tr>
            <?php
            }
            ?>
