...
<script type="text/javascript">
	$(document).ready(function($) {
		...
		$('#reset').click(function(){
				var base_url = '<?php echo $base_url.'insider_message/lists'; ?>';
		...
		$(function() {
		$.ajax({
			url:'<?php echo $base_url; ?>insider_message/ajaxlist_colour',
			type:'POST',
			data:'',
...
<div class="main-content theme">
  <ol class="breadcrumb bc-3">
    ...
    <li class="active"><strong>Insider Message</strong></li>
  </ol>
<?php if(isset($_POST))
{
if(count($result)==0)
...
<?php }  ?>
  ...
  <div role="grid" class="dataTables_wrapper form-inline" id="table-2_wrapper">
    <form action="<?php echo $base_url."insider_message/deletes1";?>" method="post" enctype="multipart/form-data" id="form">
	...
	<table id="table-2" class="table table-bordered table-striped datatable dataTable" aria-describedby="table-2_info" style="float:left;">
        <thead>
			<tr role="row" class="bg">
				...
				<th class="sorting" role="columnheader" aria-controls="table-2" style="width: auto;">Message Name</th>
			</tr>
        </thead>
        <tbody role="alert" aria-live="polite" aria-relevant="all">
			...
			<tr class="odd">
				...
				<td class=""><?php echo $this->insidermessage_model->getusername($result[$i]->userid); ?></td>
				<td class="" style="cursor;point;" onclick="window.location.href='<?php echo $base_url; ?>topics/list_message/<?php echo $result[$i]->topicid; ?>'"><?php echo $this->insidermessage_model->gettopicname($result[$i]->topicid); ?> </td>
				<td class=""><?php echo $result[$i]->message; ?></td>
			</tr>
			<?php }  } else { ?>
			<tr>
				<td class="left" colspan="4" align="center"><?php echo 'No Insider Message Found'; ?></td>
			</tr>
			<?php    }  ?>
        </tbody>
	</table>
