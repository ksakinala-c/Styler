...
class attribute_model extends CI_Model {
	...
	function getsize($id)	{		$query = "SELECT * from attribute where id = '".$id."'";		$result = $this->db->query($query);		if ($result->num_rows() > 0)		{			$result = $result->row();						return $result;		}	}		function getcolor($id)	{		$query = "SELECT * from colour where id = '".$id."'";		$result = $this->db->query($query);		if ($result->num_rows() > 0)		{			$result = $result->row();						return $result;		}	}	
