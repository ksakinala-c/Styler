...
class Status extends CI_Controller {
 	...
 	function lists()
	{
		...
		$config['base_url'] = $url_to_paging.'status/lists/?per_page='.$page.'&fname=';
		$config['per_page'] = '10';
		$config['first_url']='0';
		 
	
		$return = $this->status_model->lists($config['per_page'],$page,$data);
		...
		$this->load->view('list_status', $data);
	...
	function edit($id)
	{
		...
		if(is_numeric($id)) {
			$result = $this->status_model->get($id);
					$form_field = $data = array(
						'L_strErrorMessage' => '',
						'id'	=> $result[0]->id,
			...
			if($this->input->post('action') == 'edit_status') {
				...
				{
					$ccid=$id;
					if(!$this->status_model->is_status_already_exist($this->input->post('status'),$ccid))
					{
								//$subcatid = $this->input->post('subcatid');
								if($response = $this->status_model->edit($id, $form_field)) 
								{
	...
	function deletes()
	{
		...
		foreach($_POST['selected'] as $selCheck) {
			    if($this->status_model->deletes($selCheck)) {
	...
	function ajaxlist_status()
	{
		//echo 11;exit;
		$status = "";
		$result = $this->status_model->status_list($this->input->post('status'));
