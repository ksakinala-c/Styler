<?php
class Insider_user extends CI_Controller {
	private $_data = array();
	function __construct() {
		...
		$this->load->model('insideruser_model');	
	}
	function lists()
	{
		...
		$per_page = '1';
		$perpage = '3';
		$return = $this->insideruser_model->lists($config['per_page'],$this->uri->segment(3), $data);
		$data['result'] = $return['result'];
		$config['total_rows'] = $return['count'];
		$this->pagination->initialize($config);
		$this->load->view('list_insideruser',$data);
	}
	function referrals($id){
		$this->load->model('insideruser_model');
		$data['err_msg']='';
		$data['allinviters'] = $this->insideruser_model->allinviters($id);
	...
	/*function add()
	{
		...
		if(!isPageRefresh() || $this->input->post('action') == 'add_topics') {
			...
			if ($this->validation->run() == FALSE) {
				$data['L_strErrorMessage'] = $this->validation->error_string;
					...
					if(!$this->insideruser_model->is_colour_already_exist_add($this->input->post('topicname')))
					{
						if($response = $this->insideruser_model->add($data))
						{
		...
		$data['topic_user'] = $this->insideruser_model->topic_user();
	...
	{
		if(is_numeric($id)) {
			$result = $this->insideruser_model->get($id);
					$form_field = $data = array(
						'L_strErrorMessage' => '',
						'id'	=> $result[0]->id,
			...
			if($this->input->post('action') == 'edit_colour') {
				...
				{
					$ccid=$id;
					if(!$this->insideruser_model->is_colour_already_exist($this->input->post('topicname'),$ccid))
					{
								if($response = $this->insideruser_model->edit($id, $form_field)) 
								{
...
*/
	function deletes()
	{
		...
		foreach($_POST['selected'] as $selCheck) {
			    if($this->insideruser_model->deletes($selCheck)) {
					$this->session->set_flashdata('L_strErrorMessage','Insider User Deleted Successfully!!!!');
				}  
				else 
				{
		...
		redirect($this->config->item('base_url').'insider_user/lists');
