...
class appointment extends CI_Controller {
	...
	function lists()
	{
		...
		$config['per_page'] = '15';
		$return = $this->user_appointments_model->lists($config['per_page'],$this->uri->segment(3), $this->input->post('searchTxtName')); 
		$data['result'] = $return['result'];
		$data['hoursRange']=$this->appointment_time_model->hoursRange();
		$data['dayTimes']=$this->appointment_time_model->getDayTimes();
		$config['total_rows'] = $return['count'];
		$this->pagination->initialize($config);
		$this->load->view('user_appointments_list_view', $data);
	}
	...
	{
		$query = $this->db->query("SELECT * FROM user_appointment_time_mas where status = 1");
		$eventsInfo=$query->result();
		$i=0;
		foreach($eventsInfo as $events)
		{
			$dt = strtotime($events->appointment_date.' '.$events->start_time);
			$eventData['appointmentInfo'][$i]['title']=$events->first_name;
			$eventData['appointmentInfo'][$i]['start']=date("Y-m-d H:i:s", $dt);
			$eventData['appointmentInfo'][$i]['description']='Name '.$events->first_name.' '.$events->last_name.'<br>Email: '.$events->email.'<br>Phone: '.$events->phone_number.'<br>Appointment Location : '.$events->appointment_location;
			$i++;			
		}
		$data['appointmentInfo']=json_encode($eventData['appointmentInfo']);
		//echo "<pre>";
		//print_r($data['appointmentInfo']);exit;
		$this->load->view('display_appointments', $data);
	}
