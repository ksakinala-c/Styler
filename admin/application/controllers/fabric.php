...
class Fabric extends CI_Controller {
		...
		for($i = 0; $i < count($_FILES['attachment1']['name']); $i++)
		{
			...
			if($_FILES['attachment1']['name'][$i] != '') { 
				...
				$tmp_name1 =  $_FILES['attachment1']['tmp_name'][$i];  //$_FILES['attachment'.$i]['tmp_name'];
		 		$rootpath1 =  $this->config->item('upload')."style/";
				...
				$tmp_path = $this->config->item('upload')."style/".$logoname;
				$image_thumb= $this->config->item('upload')."style/medium/".$logoname; 
				...
				$tmp_path = $this->config->item('upload')."style/".$logoname;
				$image_thumb= $this->config->item('upload')."style/large/".$logoname; 
				...
				$tmp_path = $this->config->item('upload')."style/".$logoname;
				$image_thumb= $this->config->item('upload')."style/small/".$logoname; 
	...
	function show_category()
	{
		...
		//exit;
		$html = "<select id='subcatid' name='subcatid[]' class='form-control jobtext' multiple='multiple'>";
		$html .= "<option value='' selected='selected' >Select Sub category</option>";
		if($data != '')
		{
