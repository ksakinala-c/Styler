...
class order extends CI_Controller {
	...
	function reports()
	{
		...
		$config['base_url'] = $url_to_paging.'product/reports/';
		$config['per_page'] = '15'; 
	...
	function can_order_reports()
	{
	 	$this->load->library('pagination');
		$url_to_paging = $this->config->item('base_url');
		$config['base_url'] = $url_to_paging.'order/can_order_reports/';
		$config['per_page'] = '15'; 
		$data['username'] = $this->input->post('username');
		$data['startdate'] = $this->input->post('startdate');
		$data['enddate'] = $this->input->post('enddate');
		$return = $this->product_model->cancelorder_manage1($config['per_page'],$this->uri->segment(3), $data); 
		$data['result'] = $return['result'];
		$config['total_rows'] = $return['count'];
		$this->pagination->initialize($config); 
	...
	function pen_order_reports()
	{
	 	$this->load->library('pagination');
		$url_to_paging = $this->config->item('base_url');
		$config['base_url'] = $url_to_paging.'order/pen_order_reports/';
		$config['per_page'] = '15'; 
		$data['username'] = $this->input->post('username');
		$data['startdate'] = $this->input->post('startdate');
		$data['enddate'] = $this->input->post('enddate');
		$return = $this->product_model->cancelorder_manage2($config['per_page'],$this->uri->segment(3), $data); 
		$data['result'] = $return['result'];
		$config['total_rows'] = $return['count'];
		$this->pagination->initialize($config); 
		 
		$this->load->view('pen_order_report',$data);   
