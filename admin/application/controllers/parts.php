...
class Parts extends CI_Controller {
		...
		if(!isPageRefresh() || $this->input->post('action') == 'add_parts') {
			...
			if ($this->validation->run() == FALSE){
				...
				$data['L_strErrorMessage'] = $this->validation->error_string;
					...
					if(!$this->parts_model->is_parts_already_exist_add($this->input->post('pname'), $pid))
					{
	...
	{		
			...
			$result = $this->parts_model->get_parts($id);  
			 $data = array(
						...
						'hide_in_size' => $result[0]->hide_in_size,
	...
	function updateorder($val,$id,$pid,$segid){
		...
		redirect($this->config->item('base_url').'parts/editimage/'.$pid.'/'.$segid);
