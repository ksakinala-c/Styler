...
class Size extends CI_Controller {
 	...
 	function lists($id)
	{
		...
		$config['base_url'] = $url_to_paging.'size/lists/'.$id.'?per_page='.$page.'&name=';
		$config['per_page'] = '10';
//		$config['first_url']='0';
		$data['sizeid'] = $id;
	...
	function edit($id, $subcat)
	{
		...
		if(is_numeric($id)) {
			...
			if($this->input->post('action') == 'edit_size') {
				...
				else 
				{
					...
					if(!$this->size_model->is_size_already_exist($this->input->post('size'),$id, $subcat))
					{
								...
								if($response = $this->size_model->edit($id,$subcat, $form_field)) 
								{
			...
			$data['sizeid']= $subcat;
			$data['allparts']=$this->size_model->allparts($subcat);
	...
	function updateorder($val,$id,$pid,$segid){
		$return = $this->size_model->updateorder($id,$val);
		$this->session->set_flashdata('item', 'Size Image Updates Successfully');
		redirect($this->config->item('base_url').'size/editimage/'.$pid.'/'.$segid);
		
	}
	function setbaseimg($id,$pid,$segid,$imgindex)
	{
	...
	function removeimage($id,$pid,$segid)
	{
