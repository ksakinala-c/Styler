  ...
  function order_manage($num, $offset, $content) 
	{
		...
		{
			$sql = "SELECT o.*, u.* FROM ci_orders o left join users u ON u.id = o.user_id WHERE  o.order_id <> 0 AND o.order_status ='".$content['order_status']."'";
		}
		else
		{
			$sql = "SELECT o.*, u.* FROM ci_orders o left join users u ON u.id = o.user_id WHERE  o.order_id <> 0 AND o.order_status = 'C'";
		}
 	 ...
 	 if($content['startdate']!='' && $content['enddate']!='')
		...
		/*if($content['startdate']!='' && $content['enddate']!='')
			{
				$sDate = strtotime($content['startdate']." 00:00:00");
				$eDate = strtotime($content['enddate']." 23:59:59");
				$sDate=date("Y-m-d H:i:s", $sDate);
				$eDate=date("Y-m-d H:i:s", $eDate);
				$content['startdate']="'".$sDate."'";
				$content['enddate']="'".$eDate."'";
				$sql .= " AND ( o.cdate >= ".$content['startdate']." AND o.cdate <= ".$content['enddate']." )";
			}else if($content['startdate']!=''){
				$sDate = strtotime($content['startdate']." 00:00:00");
				$sDate=date("Y-m-d H:i:s", $sDate);				
				$content['startdate']=$sDate;				
				$sql .= " AND o.cdate >= "."'".$content['startdate']."'";
			}else{
				$eDate = strtotime($content['enddate']." 23:59:59");
				$content['enddate']=$eDate;			
				//$sql .= " AND o.cdate <= "."'".$content['enddate']."'";
			}*/
		if($content['cancel'] != '') 
		{
