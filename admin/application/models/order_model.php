...
class Order_model extends CI_Model {
	...
	function order_manage($stardate, $enddate) 
	{
			 
		$sql = "SELECT count(*) as totalorders, sum(order_total) as totalorderamount FROM ci_orders o left join users u ON u.id = o.user_id WHERE  o.order_id <> 0 AND o.order_status = 'C' ";
			if($stardate!='' && $enddate !='')
			{
				$sql .= " AND ( `cdate` >= '".$stardate."' AND `cdate` <= '".$enddate."' )";
			}
	...
	function user_manage($stardate, $enddate){
		$sql = "SELECT count(*) as totalorders FROM users where ";
			if($stardate!='' && $enddate !='')
			{
				$sql .= "  ( `joindate` >= '".$stardate."' AND `joindate` <= '".$enddate."' )";
			}
